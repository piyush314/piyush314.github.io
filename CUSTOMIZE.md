...
The CV page content is generated from the JSON Resume file at [assets/json/resume.json](assets/json/resume.json). It follows the [JSON Resume standard](https://jsonresume.org/) and is selected through the `jsonresume` setting in [\_config.yml](_config.yml). Publications are synchronized from [\_bibliography/papers.bib](_bibliography/papers.bib) during deployment, so the website and downloadable CV use the same publication list.
